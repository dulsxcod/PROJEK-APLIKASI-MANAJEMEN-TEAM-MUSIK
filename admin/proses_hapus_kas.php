...
if (isset($_GET['id'])) {
    ...
    $cek_kas = mysqli_query($conn, "SELECT * FROM kas WHERE KasID = '$id'");
    $data = mysqli_fetch_assoc($cek_kas);

    if (!$data) {
        // Jika data tidak ditemukan, kembalikan ke halaman daftar anggota
        echo "<script>
                alert('Data kas tidak ditemukan!'); 
    ...
    */
    ...
    $query_hapus = mysqli_query($conn, "DELETE FROM kas WHERE KasID = '$id'");

    if ($query_hapus) {
        // Jika berhasil dihapus, tampilkan pesan sukses dan kembali ke halaman utama anggota
        echo "<script>
                alert('Data Kas " . htmlspecialchars($data['Tanggal']) . " berhasil dihapus!'); 
                window.location = 'kas.php';
              </script>";
        exit;
    } else {
